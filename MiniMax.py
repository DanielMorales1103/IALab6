...
def miniMaxAlgo(the_player, board, useAlphaBeta=True):
    ...
    if the_player == 2:
        opponent = 1
        player = 2
    # Pasamos useAlphaBeta a findBestMove
    return findBestMove(board, useAlphaBeta)
...
def miniMax(the_board, heightMap, depth, isMax, alpha, beta, useAlphaBeta):
    score = 0
    ...
    if isMax:	
        ...
        for j in range(WIDTH):
            if heightMap[j]<HEIGHT:
                ...
                new_best = max(best, miniMax(the_board, heightMap, depth + 1, not isMax, alpha, beta, useAlphaBeta))
        ...
        for j in range(WIDTH):
            if heightMap[j]<HEIGHT:
                ...
                new_best = min(best, miniMax(the_board, heightMap, depth + 1, not isMax, alpha, beta, useAlphaBeta))
                the_board[HEIGHT-int(heightMap[j])][j] = 0
                heightMap[j] -= 1
                if useAlphaBeta:
                    # Poda alfa-beta
...
def findBestMove(the_board, useAlphaBeta):
    bestVal = -1000
    bestMove = -1
    ...
    for j in range(WIDTH):
        if heightMap[j]<HEIGHT:
            heightMap[j] += 1
            the_board[HEIGHT-int(heightMap[j])][j] = player
            moveVal = miniMax(the_board, heightMap, 0, False, -1000, 1000, useAlphaBeta)
            the_board[HEIGHT-int(heightMap[j])][j] = 0
            heightMap[j] -= 1
            if (moveVal > bestVal):				
                bestMove = j
